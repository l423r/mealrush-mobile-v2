...
export const productService = {
  ...
  searchByName: (name: string, page: number = 0, size: number = 20) =>
    apiClient.get<PaginatedResponse<Product>>(MY_FOOD_ENDPOINTS.PRODUCT_SEARCH_NAME, {
      params: { name, page, size }
    }),
  
  searchByBarcode: (barcode: string, page: number = 0, size: number = 20) =>
    apiClient.get<PaginatedResponse<Product>>(`${MY_FOOD_ENDPOINTS.PRODUCT_SEARCH_BARCODE}/${barcode}`, {
      params: { page, size }
    }),
