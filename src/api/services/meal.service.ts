...
export const mealService = {
  ...
  getMealElements: (mealId: number, page: number = 0, size: number = 50) =>
    apiClient.get<PaginatedResponse<MealElement>>(`${MY_FOOD_ENDPOINTS.MEAL_ELEMENTS_BY_MEAL}/${mealId}`, {
      params: { page, size }
    }),
  ...
  analyzePhoto: (request: PhotoAnalysisRequest) =>
    apiClient.post<PhotoAnalysisResponse>(MY_FOOD_ENDPOINTS.MEAL_ELEMENT_ANALYZE_PHOTO, request),
};