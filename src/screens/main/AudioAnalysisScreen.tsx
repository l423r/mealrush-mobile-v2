...
type AudioAnalysisScreenNavigationProp = NativeStackNavigationProp<
  ...
  'AudioAnalysis'
>;
type AudioAnalysisScreenRouteProp = RouteProp<
  MainStackParamList,
  'AudioAnalysis'
>;

interface EditableIngredient extends AnalysisIngredient {
...
const AudioAnalysisScreen: React.FC = observer(() => {
  const navigation = useNavigation<AudioAnalysisScreenNavigationProp>();
  const route = useRoute<AudioAnalysisScreenRouteProp>();
  const { mealStore, uiStore } = useStores();

  const { analysisResult, transcription, mealId } = route.params;
  ...
  const handleSave = async () => {
    try {
      console.log('💾 [AudioAnalysisScreen] handleSave начало');
      let currentMealId = mealId;

      if (!currentMealId) {
        console.log('🔎 [AudioAnalysisScreen] mealId не передан, проверяем существующие приемы пищи');
        ...
        if (existingMeals.length > 0) {
          ...
          console.log('✅ [AudioAnalysisScreen] Найдены существующие приемы пищи того же типа');
        ...
        console.log('ℹ️ [AudioAnalysisScreen] Существующих приемов пищи не найдено, создаем новый');
        const meal = await mealStore.createMeal({
          mealType,
          dateTime: mealTime.toISOString(),
        });
        currentMealId = meal.id;
        console.log('✅ [AudioAnalysisScreen] Прием пищи создан, id:', currentMealId);
      } else {
        console.log('ℹ️ [AudioAnalysisScreen] mealId передан:', currentMealId, '- добавляем к существующему');
      }
  ...
  const saveIngredientsToMeal = async (currentMealId: number) => {
    console.log('📝 [AudioAnalysisScreen] Сохранение ингредиентов в прием пищи:', currentMealId);
    ...
    console.log('🚀 [AudioAnalysisScreen] Навигация на HomeTabs > Main');
    navigation.navigate('HomeTabs', { screen: 'Main' });
  };

  const handleConfirmAddToExisting = async () => {
    console.log('✅ [AudioAnalysisScreen.handleConfirmAddToExisting] Пользователь выбрал добавить к существующему');
    ...
    if (existingMealForConfirm) {
      try {
        ...
        console.error('❌ [AudioAnalysisScreen] Ошибка при добавлении к существующему:', error);
  ...
  const handleCreateNewMeal = async () => {
    console.log('🆕 [AudioAnalysisScreen.handleCreateNewMeal] Пользователь выбрал создать новый прием');
    setShowMealTypeConfirmDialog(false);
    ...
    try {
      ...
      console.log('✅ [AudioAnalysisScreen] Прием пищи создан, id:', meal.id);
      await saveIngredientsToMeal(meal.id);
    } catch (error) {
      console.error('❌ [AudioAnalysisScreen] Ошибка при создании нового:', error);
      uiStore.showSnackbar(mealStore.error || 'Не удалось создать прием пищи', 'error');
    }
  };

  const handleCancelDialog = () => {
    console.log('❌ [AudioAnalysisScreen.handleCancelDialog] Пользователь отменил диалог');
    setShowMealTypeConfirmDialog(false);
  ...
  return (
    <View style={styles.container}>
      <Header title="Анализ аудио" showBackButton onBackPress={handleBack} />
